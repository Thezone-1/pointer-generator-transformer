...
flags.DEFINE_float("max_grad_norm", 1.2, "for gradient clipping")
...
def setup_training(model: SummarizationModel, batcher: Batcher):
...
def run_training(model, batcher, sess_context_manager, sv, summary_writer):
    ...
    with sess_context_manager as sess:
        ...
        while True:  # repeats until interrupted
            ...
            t1 = time.time()
            logging.info("seconds for training step: %.3f", t1 - t0)
...
def run_eval(model: SummarizationModel, batcher: Batcher, vocab: Vocab):
    ...
    while True:
        ...
        if batch is None:
            logging.warning("batch is none, skipping it")
            continue
        ...
        if results is None:
            logging.warning("results is none, skipping it")
            continue

        if not isinstance(results, (dict)):
            logging.warning("results is not instance of dict, skipping it")
            continue
...
if __name__ == "__main__":
    tf.compat.v1.app.run(main, sys.argv)
