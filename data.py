...
class Vocab:
    ...
    def __init__(self, vocab_file: str, max_size: int):
        ...
        self._count: int = 0  # keeps track of total number of words in the Vocab
    ...
    while True:
        ...
        for f in filelist:
            ...
            while True:
                ...
                yield example_pb2.Example.FromString(example_str)  # type: ignore
        if single_pass:
            print("example_generator completed reading all datafiles. No more data.")
            break


def article2ids(article_words: list[str], vocab: Vocab):
...
def abstract2ids(abstract_words: list[str], vocab: Vocab, article_oovs: list[str]):
...
def outputids2words(id_list: list[int], vocab: Vocab, article_oovs: list[str] | None):
...
def show_art_oovs(article: str, vocab: Vocab):
    """Returns the article string, highlighting the OOVs by placing __underscores__ around them"""
    unk_token = vocab.word2id(UNKNOWN_TOKEN)
    words = article.split(" ")
    # words = new_tokenizer(article)
    words = [("__%s__" % w) if vocab.word2id(w) == unk_token else w for w in words]
    out_str = " ".join(words)
    return out_str


def show_abs_oovs(abstract: str, vocab: Vocab, article_oovs: list[str] | None):
