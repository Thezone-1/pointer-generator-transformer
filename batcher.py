...
class Batcher:
    ...
    def __init__(self, data_path: str, vocab: data.Vocab, hps, single_pass: bool):
        ...
        self._example_queue: queue.Queue[Example] = queue.Queue(self.BATCH_QUEUE_MAX * self._hps.batch_size)
        ...
        self._example_q_threads: list[threading.Thread] = []
        for _ in range(self._num_example_q_threads):
            self._example_q_threads.append(threading.Thread(target=self.fill_example_queue))
            self._example_q_threads[-1].daemon = True
            self._example_q_threads[-1].start()

        self._batch_q_threads: list[threading.Thread] = []
        for _ in range(self._num_batch_q_threads):
            self._batch_q_threads.append(threading.Thread(target=self.fill_batch_queue))
            self._batch_q_threads[-1].daemon = True
            self._batch_q_threads[-1].start()

        # Start a thread that watches the other threads and restarts them if they're dead
        if not single_pass:  # We don't want a watcher in single_pass mode because the threads shouldn't run forever
            self._watch_thread = threading.Thread(target=self.watch_threads)
        ...
        while True:
            ...
            for idx, t in enumerate(self._example_q_threads):
                if not t.is_alive():  # if the thread is dead
                    ...
                    new_t = threading.Thread(target=self.fill_example_queue)
    ...
    def text_generator(self, example_generator):
        ...
        while True:
            ...
            try:
                ...
                article_text = str(e.features.feature["article"].bytes_list.value[0].decode())
                # the abstract text was saved under the key 'abstract' in the data files
                abstract_text = str(e.features.feature["abstract"].bytes_list.value[0].decode())
            except ValueError:
